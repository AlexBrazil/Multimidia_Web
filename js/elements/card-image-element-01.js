...
function prepararSomClique(cfg) {
  ...
  // [alterado] caminho padronizado para assets/audio (antes estava assets/sounds)
...
export function criarCardImage(element, abrirModalFn) {
  const {
    ...
    // [novo] rótulo acima do card, fora da imagem
  ...
  // [novo] wrapper externo para permitir inserir um cabeçalho (rótulo) acima do card
  const wrap = document.createElement('div');
  wrap.className = 'card-image-wrap'; // estilize via CSS conforme desejado

  // [novo] Cabeçalho/rótulo opcional acima do card (nunca sobre a imagem)
  if (interactionHint) {
    const isNoHover =
  ...
  // [igual] máscara suave nos cantos (não é sobreposição de texto)
  if (useMask) {
  ...
  // [novo] retorna o figure dentro do wrapper (com rótulo acima, se habilitado)
