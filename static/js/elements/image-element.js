/**
 * image-element.js
 * RenderizaÃ§Ã£o de elementos de imagem com suporte a legenda e fallback Flash.
 */
...
export function criarImagem(element) {
    ...
    if (element.source && element.source.toLowerCase().endsWith('.swf')) {
        ...
        warning.textContent = `ConteÃºdo interativo obsoleto (Flash: ${element.source}).`;
        return warning;
    }
    ...
    const sourceUrl = resolveImageAsset(element.source);
    img.src = sourceUrl || element.source;
    img.alt = element.searchText || element.title || 'Imagem do curso';
    img.title = element.title || '';

    // DimensÃµes especÃ­ficas se definidas
